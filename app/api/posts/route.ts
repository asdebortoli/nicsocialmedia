...
export async function POST(request: NextRequest) {
  try {
    ...
    let res = await auth(
      { allowedGroup: [USER_ROLES.ADMIN, USER_ROLES.SUPERVISOR] },
      request
    );
    if (res) return res;
    ...
    const { title, description, company, link } = Object.fromEntries(
      formData.entries()
    ) as {
      title: string;
      description: string;
    ...
    const filename = `posts/${newPost._id}.${img.type.split("/")[1]}`;

    // Upload to Vercel Blob Storage
    const imageUrl = await uploadImage(buffer, filename, img.type);
    newPost.thumbnailUrl = imageUrl;
    newPost.save();
